...
from userauth.serializers import UserSerializer, UserProfileSerializer, MyTokenObtainPairSerializer, UserProfileSearchSerializer
...
def OTP_create_send(email_linked, phone_linked):
    ...
    send_mail('Greetings from SmartLearn Team', mail_body, 'sam<utkp09@gmail.com>', [email_linked], html_message = html_content, fail_silently = False)                                                    
    return None  
...
class ProfileRecommendView(views.APIView):
    def get(self, request):
        ...
        serializer = UserProfileSearchSerializer(query, many = True, context = {'request': request})
        return Response(serializer.data, status = status.HTTP_200_OK)
