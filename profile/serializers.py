...
class SocialProfileSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self,instance):
        response = super().to_representation(instance)
        # response['user'] = UserProfileSerializer(instance.user, context = {'request': self.context.get('request')}).data
        response['first_name'] = UserProfileSerializer(instance.user, context = {'request': self.context.get('request')}).data['first_name']
        response['last_name'] = UserProfileSerializer(instance.user, context = {'request': self.context.get('request')}).data['last_name']
        response['avatar'] = UserProfileSerializer(instance.user, context = {'request': self.context.get('request')}).data['avatar']
