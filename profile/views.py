...
class SocialProfileView(views.APIView):
    ...
    def get(self, request):
        ...
        query = list()
        query.append(user)
        q = Connection.objects.filter(sender=user)
        p = Connection.objects.filter(receiver=user)
        for i in q:
            query.append(i.receiver.pk)    
        for i in p:
            query.append(i.sender.pk)    
        queryset = SocialProfile.objects.exclude(user__in=query)
...
class BannerView(views.APIView):
    ...
    def get(self, request, profile_id = None):
        ...
        try:
            domain_name = request.META['HTTP_HOST']
            picture_url = user.social_profile.background_photo.url
            absolute_url = 'http://' + domain_name + picture_url
            background = absolute_url
        except:
            background = None
        ...
        bookmarked_posts    = user.bookmarked_posts.count()
        try:
            volunteer_experience = VolunteerExperienceSerializer(user.volunteer_experience,many=True,context = {'request':request}).data 
        except:
            volunteer_experience = None
        try:
            certifications = LicenseAndCertificationSerializer(user.certifications.all(),many=True,context = {'request':request}).data 
        except: 
            certifications = None
        try:
            project = ProjectSerializer(user.project.all(),many=True,context = {'request':request}).data
        except:
            project = None
        try:
            course= CourseSerializer(user.course_taken.all(),many=True,context = {'request':request}).data 
        except:
            course=None
        try:
            test= TestScoreSerializer(user.test.all(),many=True,context = {'request':request}).data 
        except:
            test=None
        try:
            skills= SkillSerializer(user.skills,context = {'request':request}).data 
        except:
            skills=None
        ...
        return Response({'avatar':avatar,'first_name':first_name, 'last_name':last_name, 'background':background,
                         'location':location,'tagline':tagline,'experience':experience,'volunteer_experience':volunteer_experience,
                          'certifications':certifications, 'project':project,'course':course,'test':test,'skills':skills,
