...
import { myNotifications } from 'redux/actions/notifications';

const Notifications: React.FC = () => {
  const profile = useSelector((state:AppState)=>state.auth.profile_id)
  const notifications = useSelector((state:AppState)=>state.notifications.notifications)
  const  dispatch= useDispatch()
  useEffect(()=>{
    dispatch(myNotifications(profile as number))
  },[dispatch,profile])
  return(
  <Container>
    <Box title="رویدادهای اخیر">
      {notifications.map((notif,i)=>{
        return(
                <Item
        image={
          <Author image={notif.source_avatar} />
                }
        activity={(
          <span>
            {notif.detail}
            <strong> {notif.source_name}</strong>
          </span>
                )}
